const {EmbedBuilder, Collection, PermissionsBitField, ActionRowBuilder, ButtonBuilder } = require('discord.js');
const ms = require('ms');
const bot = require('..');
...
bot.on('interactionCreate', async interaction => {
    ...
    if (interaction.type == 4) {
        if (slashCommand.autocomplete) {
            const choices = [];
            await slashCommand.autocomplete(interaction, choices, bot)
        }
    }
    if (!interaction.type == 2) return;

    if (!slashCommand) return bot.slashCommands.delete(interaction.commandName);
    try {
        if (slashCommand.cooldown) {

            const cooldowntext = "You are in cooldown for another <duration>"
            if (cooldown.has(`slash-${slashCommand.name}${interaction.user.id}`)) return interaction.reply({
                ephemeral: true,
                content: cooldowntext.replace('<duration>', ms(cooldown.get(`slash-${slashCommand.name}${interaction.user.id}`) - Date.now(), {long: true}))
            })
